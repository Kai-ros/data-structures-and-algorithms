...
const filterStringsWithVowels = (arr) => {
  return arr.filter(word => {
    return word.match(/\b\w*[aeiou]\w*\b/gi);
  });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(statName => {
    if (statName.baseStat > minBaseStat) {
      return true;
    }
  });
};
...
const getStatName = (arr, minBaseStat) => {
  let arrayFilter = arr.filter(statName => {
    if (statName.baseStat > minBaseStat) {
      return true;
    }
  });

  return arrayFilter.map(value => value.stat.name);
};
...
//   let arrayFilter = arr.filter(person => {
...
const evenOddNumericValues = (arr) => {
  return arr.filter(words => typeof (words) !== 'number' ? false : true)
    .map(value => value % 2 ? 'odd' : 'even');
};
