...
const count = (target, input) => {
  ...
  for (let value of input) {
    bigArray = bigArray.concat(value);
  }
  return bigArray.filter( value => value === target ? true : false).length;
};
...
const totalSum = (input) => {
  ...
  return bigArray.reduce( ( (sum, value) => sum += value ), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.reduce( (arrayOfArrays, innerArray) => {
    arrayOfArrays.push(innerArray.filter( element => ( typeof element === 'number' ) && !( element % 5 ))
      .map( multipleOfFive => 2 ** multipleOfFive));
...
let findShortest = (data) => {
  return data.reduce( (shortestCharacter, character) => {
    if (parseInt(character.height) < parseInt(shortestCharacter.height)) 
      return character
    else
